...
d             = 0.005             #surface density (mass per unit area)
...
mSail         = A * d             #mass of membrane
m0            = 3                 #mass of payload
m             = mSail + m0;       #total mass of spacecraft
...
if (useCallback)
    ...
    sol                 = solve(prob, solver, reltol=1e-15, abstol=1e-15, maxiters=maxiters, callback=cb)
else
    sol = solve(prob, solver, reltol=1e-15, abstol=1e-15, maxiters=maxiters)
    #Other good solvers:
    #RadauIIA5()
...
#WIP
finState=sol.u[length(sol.u)]
fin_r_mag = sqrt(finState[1]^2+finState[3]^2+finState[5]^2)
fin_vel_mag = sqrt(finState[2]^2+finState[4]^2+finState[6]^2)
timeTaken = sol.t[length(sol.t)] - sol.t[length(1)]
timeToHelipause = (123*AU - fin_r_mag)/fin_vel_mag
timeRequired = timeToHelipause + timeTaken
...
# maxMass = optimiseForMass(x0,tspan,solver,p)
